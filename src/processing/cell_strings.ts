import { CellString } from "../types/cells";
 ...
 */
export const create_unary_cell_string = (char: string, width: number): CellString => {
  var cell_string: CellString = []

  for (var i = 0; i < width; i++) {
    cell_string.push({ char: char });
  }
 ...
 */
export const pad_row = (cell_string: CellString, width: number, char: string = CHAR.S): CellString => {
  cell_string.push(...create_unary_cell_string(char, width - cell_string.length));
